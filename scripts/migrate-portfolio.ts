...
// Demo projects from the original design handoff. They were never real
// Novawerk work — the canonical portfolio is the two entries in SEEDS
// (nw-knowledge-base/projects/overview.md). Listed explicitly rather than
// deleting "anything not in SEEDS", so a project an editor adds in /admin
// survives a seed run.
const RETIRED_SLUGS = [
  "tinkerlab",
  "coldchain-watch",
...
async function main() {
  ...
  for (const slug of RETIRED_SLUGS) {
    ...
    if (retired.totalDocs === 0) continue;

    await payload.delete({
      collection: "portfolio-items",
      id: retired.docs[0].id,
    });
    console.log(`- delete ${slug} (retired demo project)`);
  }
